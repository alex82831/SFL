...
**SFL — Scripting for Fun**，版本 0.6.0
...
321 个内置函数里，238 个是运行时中的 C 原语，81 个用 SFL 自己写成——集合变换、
函数组合子、持久化更新、编解码、正则引擎——预编译进一个归档，程序用到哪个就链接
哪个。哪一层实现对程序不可见。
...
共 205 个内置函数。在 REPL 里用 `:builtins <关键字>`、在脚本里用 `help("<关键字>")`
可以随时查阅同样的内容。
