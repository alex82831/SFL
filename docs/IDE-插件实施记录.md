...
| **运行配置错配修复（IJ）** | LSP4IJ 的 `DAPRunConfigurationProvider.isConfigurationFromContext` 只问「这文件可调试吗」（对每个 .sfl 都为真），于是第一个建出来的临时配置被后续所有文件复用——打开另一个 main.sfl 按 Run 跑的还是上一个。修复：`SflRunConfigurationProducer` 按 `configuration.getFile()` 与上下文文件路径比较来判定归属，并在 `shouldReplace`/`isPreferredConfiguration` 中压过对方；同时工厂**不再写 serverMappings**（那正是让对方误配的依据），改为把配置命名为 `main.sfl (项目名)`。字节码核对过：断点放置走 `DebugAdapterManager.isDebuggableFile` 的 **factory** 分支（首步 `findDebugAdapterServerFor` 命中即返回），与配置 mappings 无关，故不回归上一轮的断点修复；mappings 的其余消费者只有设置页 UI | gradle 构建 + 字节码核对 |
