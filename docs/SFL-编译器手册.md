...
未定义的名字同样会给出拼写建议，搜索范围是内置函数与程序自己的全局名字——与解释器
搜索的集合、使用的编辑距离上限都一致。
...
检验：把模块 `import` 进解释器，它的定义会遮蔽同名内置函数，然后跑 `tests/builtins.sfl`
的 236 条断言——两边必须给出相同的答案。
...
能由已有原语组合出来的，写进 `stdlib/`：写成普通 SFL，加到某个模块里，然后把模块
`import` 进解释器跑 `tests/builtins.sfl`——你的实现会遮蔽原生版本，两者必须给出相同的
答案。编译器会自动发现它（扫描 `def`），并在程序用到时链接对应的目标文件。
