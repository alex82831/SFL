...
| 功能 | 来源 |
| --- | --- |
| 语法高亮、注释切换、括号配对 | 插件内置的小词法器（关键字、内置函数与命名空间词表由 `sfl syntax` 生成）；`ns.member` 单独成色 |
| 诊断、补全、悬停、签名提示 | `sfl lsp`（通过 LSP4IJ 接入） |
| `.` 成员补全（命名空间、包单元、对象键）/ `import "` 路径补全 / Structure 视图 | 同一个 `sfl lsp`——服务器声明触发字符与能力，插件零改动获得 |
...
| **配置页** | Settings → Build, Execution, Deployment → Build Tools → **SFL**：已链接项目列表（增删）、自动发现开关；机器级设置仍在 Tools → SFL |
